...
(function () {
  ...
  const { createLocalStorage } = require('../core/storage');
  const { createModal, createToast, injectStyle, positionFloatingPanel } = require('../core/ui');
  const { createDeepSeekAdapter } = require('../adapters/deepseek');
  const { exportFileName, renderMarkdownMessageTree } = require('../features/export/format');
  ...
  function saveCats(data) { localStore.setJson(LS_CATS, data); }
  ...
  function renderBatchFailures(statusEl, failures, retryLabel, retryFn) {
    if (!failures.length) { statusEl.style.display = 'none'; return; }
  ...
  async function runBatchDelete(ids) {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      try { await apiDelete(ids[i]); ok++; }
      catch (e) { failures.push({ id: ids[i], title: session?.title || ids[i], error: e.message }); }
    }
  ...
  function showForkPicker(sessionId, messages) {
    ...
    userMsgs.forEach((m, i) => {
      ...
      r.onclick = () => { listEl.querySelectorAll('.dse-msg-row').forEach(e => e.classList.remove('sel')); r.classList.add('sel'); sel = i; updateRange(); };
  ...
  function renderCatChips() {
    ...
    catData.categories.forEach(c => {
      ...
      chip.querySelector('.x').onclick = (e) => { e.stopPropagation(); if (confirm(`删除分类「${c.name}」？`)) { removeCategory(c.id); renderCatChips(); renderCatFilterBar(); renderCatBulkOptions(); renderSearchCatOptions(); nativeMountManager.schedule(); } };
  ...
  function applyBulkCategory(mode) {
    ...
    if (!ids.length) { toast('请先选择当前列表中的对话', 'error'); return; }
    let changed = 0;
    ids.forEach(id => {
  ...
  function showCatPicker(sid) {
    ...
    catData.categories.forEach(c => {
      ...
      r.onclick = () => { toggleCatSession(sid, c.id); nativeMountManager.schedule(); showCatPicker(sid); close(); };
  ...
  panel.querySelector('#cat-add').onclick = () => {
    ...
    renderCatChips(); renderCatFilterBar(); renderCatBulkOptions(); renderSearchCatOptions();
    nativeMountManager.schedule();
    toast(`已添加「${name}」`, 'success');
  };

  panel.querySelector('#cat-load').onclick = async () => {
    try { catListEl.innerHTML = '<div style="color:#888;padding:8px 0">加载中...</div>'; allSessions = await fetchAllSessions(); selIds.clear(); renderCatFilterBar(); renderCatListFiltered(); toast(`已加载 ${allSessions.length} 条`, 'success'); }
    catch (e) { toast(`失败: ${e.message}`, 'error'); }
  };
  panel.querySelector('#cat-sel-all').onclick = () => { getVisibleCatSessions().forEach(s => selIds.add(s.id)); renderCatListFiltered(); };
  panel.querySelector('#cat-desel').onclick = () => { selIds.clear(); renderCatListFiltered(); };
  ...
  panel.querySelector('#cat-import-data').onclick = () => {
    ...
    inp.onchange = async () => {
      ...
      try {
        ...
        renderCatChips(); renderCatFilterBar(); renderCatBulkOptions(); renderSearchCatOptions(); nativeMountManager.schedule();
  ...
  panel.querySelector('#search-load').onclick = async () => {
    try { searchListEl.innerHTML = '<div style="color:#888;padding:8px 0">加载中...</div>'; allSessions = await fetchAllSessions(); renderSearchCatOptions(); doSearch(); toast(`已加载 ${allSessions.length} 条`, 'success'); }
    catch (e) { toast(`失败: ${e.message}`, 'error'); }
  };
  ...
  function exportCurrentSelectionFragment() {
    ...
    if (!text) { toast('请先选中要导出的消息片段', 'error'); return; }
    const date = new Date().toISOString().slice(0, 10);
  ...
  function exportMessageFragment(messageEl) {
    ...
    if (!text) { toast('该消息没有可导出的文本', 'error'); return; }
    const date = new Date().toISOString().slice(0, 10);
    const sid = (getSessionId() || 'current').replace(/[^a-zA-Z0-9_-]+/g, '-');
  ...
  async function runRenameBatch(renames, rerender) {
    ...
    for (let i = 0; i < renames.length; i++) {
      ...
      try { await apiRename(renames[i].id, renames[i].title); ok++; }
      catch (e) { failures.push({ ...renames[i], error: e.message }); }
    }
    toast(`完成: 成功 ${ok}, 失败 ${failures.length}`, ok ? 'success' : 'error');
    allSessions = await fetchAllSessions();
    rerender();
  ...
  panel.querySelector('#rnm-go').onclick = async () => {
    ...
    if (mode === 'direct') {
      ...
      runRenameBatch(renames, () => renderDirectRenameList(allSessions));
      return;
    }
    ...
    promptListEl.querySelectorAll('.dse-pcard').forEach(card => {
      ...
      card.querySelector('.p-up').onclick = () => {
        ...
        if (idx > 0) {
          ...
          savePrompts(pList); renderPromptCards(); InlinePromptUI.update();
        }
      };
      ...
      card.querySelector('.p-save-content').onclick = () => {
        ...
        const pList = loadPrompts();
        const p = pList.find(x => x.id === id);
        if (p) { p.content = val; p.group = group; savePrompts(pList); }
        toast('内容已保存', 'success');
