...
2. 编辑 `mcp.json` 配置 Bing API Key（可选）：
   ```json
   {
     "services": {
       "web_search": {
...
3. 启动服务器：
   ...
   python server.py
   # 或指定日志文件
   python server.py --log-file mcp.log
   ```
...
| 工具 | 说明 |
|------|------|
| `execute_command` | 执行 Shell 命令（白名单限制） |
| `get_cwd` | 获取当前工作目录 |
...
**路径限制：** 文件操作仅限工作目录内。可通过环境变量自定义：
```bash
export DS_WORKSPACE=/path/to/your/project
python server.py
```
